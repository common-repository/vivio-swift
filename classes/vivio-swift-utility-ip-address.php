...
class Vivio_Swift_Utility_IP
{
    ...
    static function get_sanitized_ip_range($ip)
    {
        ...
        else
        {
            ...
            $vivio_swift_global->debug_logger->log_debug("Vivio_Swift_Utility_IP - Invalid IP received ".$ip,4);
        }
        return $ip_range;
    }
            ...
            foreach($submitted_ips as $item) 
            {
                ...
                if (strlen( $item ) > 0) 
                {
                    ...
                    if(strpos($item, ':') !== false){
                        ...
                        if(FALSE === $res){
                            $errors .= "\n".$item.__(' is not a valid ip address format.', 'vivio-swift');
                        }else if($res == '6'){
                            $list[] = trim($item);
                        }
                        continue;
                    ...
                    {
                        $current_user_ip = Vivio_Swift_Utility_IP::get_user_ip_address();
                        if ($current_user_ip == $item && $list_type == 'blacklist')
                        {
                            //You can't ban your own IP
                            $errors .= "\n".__('You cannot ban your own IP address: ', 'vivio-swift').$item;
                        }
                        else
                        {
                            $list[] = trim($item);
     ...
     * @global type $vivio_swift_global
    ...
    static function is_ip_whitelisted($ip_address, $whitelisted_ips){
        ...
        $ip_list_array = Vivio_Swift_Utility_IP::create_ip_list_array_from_string_with_newline($whitelisted_ips);
